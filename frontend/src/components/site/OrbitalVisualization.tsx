...
export function OrbitalVisualization({ nodes }: { nodes: NodeData[] }) {
  ...
  // On desktop, radius is 220, box is 500, icon is 80
  // On mobile, radius is 140, box is 320, icon is 50
  const RADIUS = { desktop: 220, mobile: 140 };
  const BOX = { desktop: 500, mobile: 320 };
  ...
  return (
    <div className="relative mx-auto flex w-full flex-col items-center justify-center overflow-hidden">
      {/* Desktop Orbit */}
      <div className="relative z-0 hidden w-full max-w-[1000px] flex-col items-center justify-center h-[500px] md:flex">
        <div
          className="absolute left-1/2 top-0 -translate-x-1/2"
          style={{ width: BOX.desktop, height: BOX.desktop }}
        >
          {/* Dashed track */}
          <div
          ...
          >
            {nodes.map((node, i) => {
              ...
              return (
                <motion.div
                  key={node.id}
                  className="absolute flex items-center justify-center rounded-full bg-background shadow-[0_8px_20px_-8px_rgba(0,0,0,0.15)] ring-1 ring-border p-3"
        ...
        {/* Central Logo */}
        <div className="absolute left-1/2 top-[250px] -translate-x-1/2 -translate-y-1/2 flex items-center justify-center">
            ...
            >
              {nodes.map((node, i) => {
                ...
                return (
                  <motion.div
                    key={node.id}
                    className="absolute flex items-center justify-center rounded-full bg-background shadow-[0_8px_20px_-8px_rgba(0,0,0,0.15)] ring-1 ring-border p-2 -translate-x-1/2 -translate-y-1/2"
...
function NodeCard({ node }: { node: NodeData }) {
  const content = (
    <div className="group relative flex h-full w-full cursor-pointer items-center justify-center transition-transform hover:scale-110">
      <div className="h-full w-full rounded-full overflow-hidden flex items-center justify-center">
        {node.logo ? (
          <img src={node.logo} alt={node.name} className="h-full w-full object-contain" />
        ) : (
      ...
      {/* Tooltip */}
      <div className="pointer-events-none absolute -bottom-10 left-1/2 -translate-x-1/2 whitespace-nowrap bg-foreground px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-background opacity-0 shadow-xl transition-opacity group-hover:opacity-100 z-50">
        {node.name}
      </div>
    </div>
  );
  ...
  if (node.website_link) {
    return (
      <a
        href={
          node.website_link.startsWith("http") ? node.website_link : `https://${node.website_link}`
        }
        target="_blank"
        rel="noreferrer"
        className="block size-full outline-none hover:-translate-y-1 transition-transform"
      >
        {content}
      </a>
    );
  ...
  return <div className="size-full hover:-translate-y-1 transition-transform">{content}</div>;
}
