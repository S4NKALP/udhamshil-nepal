  ...
  return (
    <div className="relative bg-background">
          ...
          <div className="mx-auto max-w-7xl px-6">
            <div className="grid lg:grid-cols-12 gap-12 items-end mb-16">
              <div className="lg:col-span-9 rise">
                <h1 className="font-display text-[12vw] sm:text-[8vw] lg:text-[6rem] leading-[0.85] tracking-tighter uppercase">
                  {hero.title} <br />
                  {hero.subtitle && <span className="text-brand block">{hero.subtitle}</span>}
                </h1>
              </div>
              <div className="lg:col-span-3 pb-4 rise-2">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: hero.hero_section }}
                />
                <div className="mt-8 flex flex-col gap-3">
                  <Link
                    to="/products"
                    className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-foreground hover:text-brand transition-colors"
                  >
            ...
            {stats.length > 0 && (
              <div className="mt-24 grid grid-cols-2 md:grid-cols-4 gap-8 border-t border-border pt-8 rise-3">
                {stats.map((st) => (
                  <div key={st.id}>
                    <p className="font-display text-4xl md:text-5xl font-black tracking-tighter">
                      <AnimatedStat value={st.stats} />+
                    </p>
      ...
      {sisterCompanies.length > 0 && (
        <section className="relative z-10 overflow-hidden border-t border-border pt-16 pb-32">
          <div className="mx-auto max-w-7xl px-6">
            <Reveal>
              <div className="mx-auto max-w-4xl text-center mb-16">
                ...
                <p className="mt-8 text-sm font-medium uppercase tracking-widest text-muted-foreground">
                  A growing network designed to support enterprise from machinery to long-term
                  operations.
                </p>
              </div>
            </Reveal>
              ...
              {marquee.map((m, i) => {
                ...
                return (
                  <Reveal
                    key={m.id}
                    delay={i * 50}
                    className={`${spanClass} bg-muted p-8 flex flex-col justify-between group overflow-hidden relative border border-border/50`}
                  >
                    ...
                    <span className="relative z-10 text-2xl font-display font-black text-muted-foreground/30 group-hover:text-background/30 transition-colors duration-700">
                      0{i + 1}
                    </span>
                    <h3 className="relative z-10 text-3xl md:text-4xl lg:text-5xl font-display font-black uppercase tracking-tight text-foreground group-hover:text-background transition-colors duration-700 break-words">
                      {m.title}
                    </h3>
                  </Reveal>
                );
            ...
            <div className="grid gap-24">
              {whatWeDo.map((c, i) => (
                <Reveal key={c.id} delay={i * 50}>
                  <div
                    className={`grid gap-12 items-center lg:grid-cols-12 ${i % 2 !== 0 ? "lg:rtl" : ""}`}
                  >
                    ...
                    <div className={`lg:col-span-5 ${i % 2 !== 0 ? "lg:ltr" : ""}`}>
                      <span className="font-display text-4xl font-black text-brand/20">
                        {(i + 1).toString().padStart(2, "0")}
                      </span>
                      <h3 className="mt-4 text-3xl font-display font-black uppercase tracking-tight">
                        {c.title}
                      </h3>
                      <div
                        className="mt-6 text-base leading-relaxed text-muted-foreground prose prose-invert"
                        dangerouslySetInnerHTML={{ __html: c.what_we_do }}
                      />
                    </div>
                  </div>
                </Reveal>
            ...
            <Reveal>
              <div className="grid gap-12 lg:grid-cols-12 lg:items-start">
                <div className="lg:col-span-5">
                  <h2 className="text-4xl font-display font-black tracking-tighter uppercase md:text-5xl">
                    From requirement
                    <br />
                    to reliable output.
                  </h2>
                  <p className="mt-6 max-w-sm text-sm font-medium leading-relaxed text-muted-foreground uppercase tracking-widest">
                    One accountable team helps you choose, receive and keep the right machinery
                    running.
                  </p>
                </div>
                    ...
                    {services.map((step, index) => (
                      <Reveal key={step.id} delay={index * 50}>
                        <div className="group grid grid-cols-1 sm:grid-cols-[auto_1fr] gap-6 sm:gap-12 py-10 border-b border-border transition-colors hover:bg-muted/30">
                          ...
                          <div>
                            <h3 className="text-2xl font-display font-black tracking-tight uppercase group-hover:text-brand transition-colors">
                              {step.name}
                            </h3>
                            <div
                              className="mt-4 text-base leading-relaxed text-muted-foreground prose prose-invert max-w-xl"
